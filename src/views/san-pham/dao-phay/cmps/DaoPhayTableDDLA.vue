...
<template>
  ...
  >
    ...
    <Column v-if="showCuttingLength" field="cuttingLength" :sortable="sortable">
      <template #header>
    ...
    <Column field="noseAngle" :sortable="sortable">
      <template #header>
        <div class="block sm:hidden">α</div>
        <div class="hidden sm:block">{{ $t("noseAngle") }} α</div>
      </template>
    </Column>
  </DataTable>
</template>
